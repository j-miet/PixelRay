...
public class Triangle(Vec3 v1, Vec3 v2, Vec3 v3, ColorRGB color) : IHittable
{
    ...
    public bool Hit(Ray ray, double tMin, double tMax, out HitRecord hit)
    {
        ...
        if (Math.Abs(determinant) < 0.0001) // if ray and normal are parallel
            return false;
        ...
        if (b < 0.0001 || b - 1 > 0.0001) // check convex combination requirements
            return false;
        ...
        if (c < 0.0001 || b + c - 1 > 0.0001) // check convex combination reqs
            return false;
        ...
        hit.Point = ray.At(t);
        hit.Normal = Vec3.Cross(e1, e2).Unit();
        hit.T = t;
        hit.Color = Color;
