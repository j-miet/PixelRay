...
public class TorusT : IHittable
{
    ...
    public Vec3 Axis
    {
        ...
        {
            _vY = _vY.Unit();
            Utils.BuildOrthonormalBasisFromY(out _vX, _vY, out _vZ);
        }
    }
    ...
    public TorusT(Vec3 center, Vec3 axis, double majorRadius, double minorRadius, ColorRGB color)
    {
        Center = center;
        MajorR = majorRadius;
        MinorR = minorRadius;
    ...
    public bool Hit(Ray ray, double tMin, double tMax, out HitRecord hit)
    {
        ...
        Ray localRay = new(O, D);

        // bounding sphere test = check if ray hits the sphere surrounding torus
        if (!Utils.HitBoundingSphere(localRay, MajorR + MinorR))
            return false;
    ...
    private Vec3 CalculateNormal(Vec3 point)
    {
        ...
        return gradLocal.X * _vX + gradLocal.Y * _vY + gradLocal.Z * _vZ;
    }
    ...
    private Vec3 PointToLocal(Vec3 p)
    {
        ...
        return new(Vec3.Dot(d, _vX), Vec3.Dot(d, _vY), Vec3.Dot(d, _vZ));
    }

    /// <summary>
    /// Transform a direction vector to torus local coordinates
    /// </summary>
    private Vec3 DirectionToLocal(Vec3 d) => new(Vec3.Dot(d, _vX), Vec3.Dot(d, _vY), Vec3.Dot(d, _vZ));
}